...
- S = 1
- Fraction = $$1000\ldots00_2$$ （$1.1_2$ 的小数部分 "1" 后补零）
- 实际指数 = -1
- 单精度：指数域 = -1 + 127 = 126 = $$01111110_2$$
  结果：**$1\_01111110\_1000\ldots00$**
- 双精度：指数域 = -1 + 1023 = 1022 = $$01111111110_2$$
  结果：**$1\_01111111110\_1000\ldots00$**

## 示例 2：浮点转十进制 (FP to Decimal)
**单精度浮点数：$$11000000101000\ldots00$$**
- S = 1
...
- **±0**：指数和尾数全为 0，符号位区分正负。
- **±$\infty$ (Infinity)**：指数全 1，尾数全 0。通常由除以 0 产生。
...
- 最小规格化数：$$1.0000\ldots0_2 \times 2^{-126}$$
- 更小的非规格化数：$$0.0000\ldots001_2 \times 2^{-126}$$，允许数值逐渐趋近于 0，避免直接下溢为 0。
...
5. 符号：正 $\times$ 正 → 正，结果 $$+1.021 \times 10^6$$
...
3. 结果已规范化，无溢出/下溢
4. 舍入：无需改变
5. 符号：正 $\times$ 负 → 负，最终 $$-1.110_2 \times 2^{-3} = -0.21875_{10}$$
...
### 精确算术：保护位与舍入
为提高中间结果精度，IEEE 754 采用额外的位：**保护位 (guard bit)**、**舍入位 (round bit)**、**粘滞位 (sticky bit)**，确保舍入结果在最后一位上的误差不超过 $\frac{1}{2} \text{ ulp}$ (units in the last place)。
- 舍入模式：
  - 向正无穷舍入（round up）
  - 向负无穷舍入（round down）
  - 向零截断（truncate）
...
### 浮点示例：矩阵乘法
**C 代码**（未优化，三重循环计算 $C = C + A \times B$，方阵大小 $32 \times 32$，双精度）：
```c
